...
export async function listAllFixes(
	...
	const conditions = [isNull(generatedFixes.deletedAt)];
 ...
 *   2. UPDATE the row with the encrypted envelope using AAD(id)
 */
export async function createCmsConnection(
	...
	return db.transaction(async (tx) => {
		...
		const aad = cmsCredentialsAad(row.id);
...
/**
 * Lists active instance-wide connections. Strips `encryptedCredentials`
 * from the result so callers cannot accidentally surface ciphertext.
 */
...
export async function getDecryptedCmsConnection(
	...
	const aad = cmsCredentialsAad(connection.id);
...
/**
 * Lists all fixes in the instance, each joined with its latest deployment
 * attempt and the latest validation for that deployment. Used by the fix
 * overview page.
 */
export async function listFixesWithLatestDeployment(
	...
	const fixes = await db
		.select()
		.from(generatedFixes)
		.where(isNull(generatedFixes.deletedAt))
		.orderBy(desc(generatedFixes.createdAt));
