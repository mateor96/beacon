...
export function AuditForm({ variant = "hero" }: AuditFormProps) {
	...
	async function handleSubmit(e: FormEvent) {
		...
		try {
			...
			const res = await fetch("/api/public/audit", {
				...
				body: JSON.stringify({ url: normalized, website_url_confirm: honeypot }),
	...
	return (
		<form onSubmit={handleSubmit} className={`mx-auto ${isCompact ? "max-w-md" : "max-w-xl"}`}>
			...
			<div className="flex flex-col gap-3 sm:flex-row">
				<label htmlFor="audit-url-input" className="sr-only">
					Website-URL
				</label>
				<input
					id="audit-url-input"
