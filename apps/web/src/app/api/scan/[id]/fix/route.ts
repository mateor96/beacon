...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		const rateLimitResult = await checkRateLimit(`fix-status:${ip}`, RATE_LIMIT_CONFIG);
		...
		const fixStatuses =
			scan.fixStatuses && typeof scan.fixStatuses === "object" && !Array.isArray(scan.fixStatuses)
				? (scan.fixStatuses as Record<
						string,
						{ status: string; error?: string; startedAt?: string; completedAt?: string }
					>)
				: null;

		return NextResponse.json(
			{ scanId: scan.id, url: scan.url, fixStatuses },
