import { serializeScan } from "@/app/results/[id]/get-scan";
import { checkRateLimit } from "@/lib/rate-limit";
import { getScanAccess, normalizeAccessToken } from "@/lib/scan-access";
import { type NextRequest, NextResponse } from "next/server";

const RATE_LIMIT_CONFIG = { maxRequests: 60, windowMs: 60_000 };

const SECURITY_HEADERS = {
	"Cache-Control": "private, no-store",
	"X-Robots-Tag": "noindex, nofollow",
	"X-Content-Type-Options": "nosniff",
};

export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		const forwarded = request.headers.get("x-forwarded-for");
		const ip = forwarded?.split(",")[0]?.trim() ?? "unknown";
		const rateLimitResult = await checkRateLimit(`scan-status:${ip}`, RATE_LIMIT_CONFIG);
		if (!rateLimitResult.allowed) {
			return NextResponse.json(
				{ error: "Zu viele Anfragen. Bitte versuche es später erneut." },
				{
					status: 429,
					headers: {
						...SECURITY_HEADERS,
						"Retry-After": String(
							Math.max(1, Math.ceil((rateLimitResult.resetAt - Date.now()) / 1000)),
						),
					},
				},
			);
		}

		const { id } = await params;
		const { UuidSchema } = await import("@beacon/shared");
		const parseResult = UuidSchema.safeParse(id);
		if (!parseResult.success) {
			return NextResponse.json(
				{ error: "Ungültige Scan-ID" },
				{ status: 400, headers: SECURITY_HEADERS },
			);
		}

		const { db, scanQueries } = await import("@beacon/db");
		const scan = await scanQueries.getStatusById(db, parseResult.data);
		if (!scan) {
			return NextResponse.json(
				{ error: "Scan nicht gefunden" },
				{ status: 404, headers: SECURITY_HEADERS },
			);
		}

		const url = new URL(request.url);
		const accessValues = url.searchParams.getAll("access");
		const accessToken = normalizeAccessToken(
			accessValues.length === 1 ? accessValues[0] : accessValues,
		);
		const accessMode = getScanAccess(scan, { viewerUserId: null, accessToken });
		if (!accessMode) {
			return NextResponse.json(
				{ error: "Scan nicht gefunden" },
				{ status: 404, headers: SECURITY_HEADERS },
			);
		}

		return NextResponse.json(serializeScan(scan, accessMode), {
			headers: SECURITY_HEADERS,
		});
	} catch (_error) {
		return NextResponse.json(
			{ error: "Ein Fehler ist aufgetreten. Bitte versuche es später erneut." },
			{ status: 500, headers: SECURITY_HEADERS },
		);
	}
}
